...
if not hisseler:
    ...
    c2.metric("Güncel Değer", f"{t_deger:,.0f} TL", delta=round(g_kar, 0), delta_color="normal" if g_kar != 0 else "off")
    c3.metric("Toplam Getiri", f"%{g_yuzde:.2f}", delta=round(g_yuzde, 2), delta_color="normal" if g_yuzde != 0 else "off")
